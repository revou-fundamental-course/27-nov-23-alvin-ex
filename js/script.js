document.getElementById('calculatePerimeterBtn').addEventListener('click', function () {
    ...
    var isIntegerInput = Number.isInteger(side1) && Number.isInteger(side2) && Number.isInteger(side3);
    ...
    perimeterResultsDiv.innerHTML = `
        ...
        <p><strong>K = ${isIntegerInput ? perimeter.toFixed(0) : perimeter.toFixed(2)}</strong></p>
    `;
});
...
document.getElementById('calculateAreaBtn').addEventListener('click', function () {
    ...
    var formattedArea = area % 1 === 0 ? area.toFixed(0) : area.toFixed(2);
    ...
    areaResultsDiv.innerHTML = `
        ...
        <p><strong>L = ${formattedArea}</strong></p>
    `;
});
